...
class YowsupCliLayer(Cli, YowInterfaceLayer):
    ...
    @clicmd("Automatically send read receipts")
    def message_autoread(self, enable):
        self.sendReceipts = enable
        ...
        for m in self.inBox[:]:
            if m.getId() == message_id:
                self.toLower(OutgoingReceiptProtocolEntity(
                    m.getId(), m.getFrom(), True, participant=m.getParticipant(),
                    t=m.getTimestamp()))
                self.output("Sent read receipt",
                            tag="Message %s" % m.getId())
                self.inBox.remove(m)
                break
        else:
            logger.error("Message %s is not in inbox" % message_id)
    ...
    def message_delivered(self, message_id):
        for m in self.inBox[:]:
            if m.getId() == message_id:
                self.toLower(OutgoingReceiptProtocolEntity(
                    m.getId(), m.getFrom(), False, participant=m.getParticipant(),
                    t=m.getTimestamp()))
                self.output("Sent delivered receipt",
                            tag="Message %s" % m.getId())
                self.inBox.remove(m)
                break
        else:
    ...
    @clicmd("Pauses script execution for specified time")
